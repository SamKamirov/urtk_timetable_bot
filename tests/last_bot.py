...
import pandas as pd
from apus.API_TOKEN import CURRENT_API_TOKEN
...
date_text = ''
WEEKDAY = dt.datetime.today().date().weekday()
    ...
    if message.text in GROUPS:
        ...
        for i in range(len(current_course)):
            if current_course[i] in DAYS:
                ...
                current_course[i] = '\n<strong><i>{}</i></strong>'.format(current_course[i])
                index = 0
            else:
                index += 1
                current_course[i] = '{}  {}'.format(index, current_course[i])
        ...
        # print(current_course)
        await bot.send_message(chat_id=message.from_user.id,
                               text=f'{date_text}\n<b>Расписание группы {message.text}:</b>'
